...
import com.example.vehiclessale.orders.ItemOrderAdapter
import com.google.firebase.database.*
import com.google.gson.Gson

class ToConfirmFragment : BaseFragment() {

    @BindView(R.id.rvConfirming)
    lateinit var rvConfirming: RecyclerView
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val view = inflater.inflate(R.layout.fragment_to_confirm, container, false)
        ...
        reference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (data in snapshot.children) {
                    ...
                    cartProduct?.forEach {
                        if (idUser == user.id && it.product.status == MyEnum.ADDED_CART.Name() && status == MyEnum.CONFIRMATION.Name()) {
