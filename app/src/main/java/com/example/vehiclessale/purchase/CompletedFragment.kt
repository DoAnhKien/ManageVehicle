...
class CompletedFragment : BaseFragment() {
    ...
    @BindView(R.id.rvPurchaseComplete)
    lateinit var rvPurchaseComplete: RecyclerView
        ...
        reference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (data in snapshot.children) {
                    ...
                    cartProduct?.forEach {
                        if (idUser == user.id && it.product.status == MyEnum.COMPLETE.Name() && status == MyEnum.COMPLETE.Name()) {
